...
const EditForm = ({ userInfo, setWarning, editUser }: any) => {
    ...
    function handleSubmit(e: FormEvent) {
        ...
        if (user.password.value.length > 0 && (user.password.value.length < 9 || !regex.test(user.password.value))) {
        ...
        const image = user.image.value.name.length > 0 ? { image: user.image.value } : { noImage: true }
        ...
        <div className={styles.form}>
            ...
            <input
                ...
                onClick={() => {
                    ...
                    setPreview(null)
                    setWarningMessage('success', 'Imagem excluida com sucesso!')
                }}
            />
            ...
            <p className={styles.form__link}>Deletar conta? <Link to='/profile/delete'>Clique aqui!</Link></p>
        </div>
    )
}
